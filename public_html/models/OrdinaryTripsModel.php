...
class OrdinaryTripsModel extends BaseModel {
	...
	public function getItems($options=null) {
		...
		$query = "SELECT sp.id, sp.id AS startPlaceId, fp.id AS finishPlaceId, sp.aliase AS startPlace, fp.aliase AS finishPlace ".
			"FROM {$this->getTable()} t ".
			"LEFT JOIN places sp ON t.startPlaceId = sp.id AND sp.lang='{$user['language_code']}' ".
			"LEFT JOIN places fp ON t.finishPlaceId = fp.id AND fp.lang='{$user['language_code']}' ".
			"WHERE (sp.id IS NOT NULL OR fp.id IS NOT NULL) ".
			"ORDER BY t.`sort`";
